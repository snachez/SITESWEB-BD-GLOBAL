...
CREATE PROCEDURE [dbo].[usp_HabilitarDepartamento](@JSON_IN VARCHAR(MAX) = NULL,
	                                             @JSON_OUT  VARCHAR(MAX) OUTPUT )
AS
BEGIN

	  ---Declaracion Variables Mensajes
      DECLARE @MetodoTemporal VARCHAR(MAX) = 'usp_HabilitarDepartamento';
		 ...
		 BEGIN TRANSACTION ACTUALIZAR
				...
				------------------------------ RECORRIDO Y SETEO DE DATA DE LA TABLA  ------------------------------------
					...
					BEGIN
						...
						SET @p_Activo_Cursor = IIF(@p_Activo_Cursor > 0, 0, 1)
						
						UPDATE tblDepartamento SET Activo = @p_Activo_Cursor, FechaModificacion = CURRENT_TIMESTAMP WHERE Id = @p_Id_Departamento_Cursor
